...
const DropDownMenu = (props) => {
  ...
  const itemClasses = {
    ...
    subNavItems:
      "mt-2 w-full origin-top-right rounded-md bg-black shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none",
  };
  1;
              ...
              <Menu.Item key={nav.label + i}>
                {({ active }) => (
                  <Link
                    ...
                    } bg-black text-white group flex w-content items-center border-bottom rounded-md px-2 py-2 text-sm`}
                  >
                    {nav.label}
                  </Link>
                )}
